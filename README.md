...
Testing will be done with the Jest framework
...
Development has begun on this project. The 'alpha' of the back end is in place and the front end is getting underway. The following is how the project will be developed:

- A basic template is designed for how the page should look and how user interactions should work.
- The file structure will be laid out and updated as development progresses
- Before a component is developed, unit tests will be written to test against to confirm components are being developed as intended (this is called Test Driven Development, or TDD)
- Once tests are in place, functions will be developed to fulfill the requirements set by the tests
- Once that component passes all of its tests, we move on to the next component and start from step 3 again.

Once a base form has been constructed, upload builds to test on netlify. Backend functionality will not work there so full functionality will have to be tested on pair.

### Schedule

There is much to learning the development of this project. The schedule will be difficult to set concretely but a general idea for the steps are below:
